        ...
        for threshold in thresholds:
            ...
            for pair in pairsTest:
                ...
                if False:
                    ...
                    # cosine distance
                    dist = pairwise_distances(x1.reshape(1, -1), x2.reshape(1, -1), metric='cosine')
                    predict_same = dist[0][0] < threshold
...
def plotVerifyExp(workDir, tag):
    ...
    openbrData = pd.read_csv("/root/openface/evaluation/comparisons/openbr.v1.1.0.DET.csv")
    ...
    humanData = pd.read_table(
        "/root/openface/evaluation/comparisons/kumar_human_crop.txt", header=None, sep=' ')
    humanPlot, = plt.plot(humanData[1], humanData[0])
    humanAUC = getAUC(humanData[1], humanData[0])

    deepfaceData = pd.read_table(
        "/root/openface/evaluation/comparisons/deepface_ensemble.txt", header=None, sep=' ')
    ...
    eigData = pd.read_table(
        "/root/openface/evaluation/comparisons/eigenfaces-original-roc.txt", header=None, sep=' ')
