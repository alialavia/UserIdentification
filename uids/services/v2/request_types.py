...
class ImageIdentificationPrealigned:
    ...
    def __init__(self, server, conn, handle):
        ...
        is_save_set, current_samples = server.classifier.id_controller.try_to_identify(tracking_id, new_samples=embeddings)

        # predict user id
        id_pred, confidence = server.classifier.predict(current_samples)
        ...
        if is_save_set:
            ...
            r.Identification(server, conn, int(id_pred), user_name, confidence=confidence, profile_picture=profile_picture)
        else:
            # return prediction and confidence - but no identification
            r.IdentificationFeedback(server, conn, int(id_pred), user_name, confidence=confidence)
